...
{
    public static IServiceCollection AddCinemaClient(
        this IServiceCollection services,
        IConfiguration configuration
    )
    {
        var endpoint =
            configuration.GetSection("CinemaGrpc:Endpoint").Value
            ?? throw new MissingFieldException("CinemaGrpc:Endpoint");

        services
            .AddGrpcClient<ShowTimeApi.ShowTimeApiClient>(
                (services, options) =>
                {
