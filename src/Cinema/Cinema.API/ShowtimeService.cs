...
namespace Cinema.Api
{
    public class ShowtimeService : ShowTimeApi.ShowTimeApiBase
    {
        ...
        public override async Task<responseModel> CreateShowTime(
            ShowtimeCreationRequest request,
        ...
        )
        {
            ...
            {
                _logger.LogDebug(
                    "New Request received on {grpcServiceName}",
                    nameof(ShowtimeService)
                );

                var response = await _mediator.Send(
                    _mapper.ApiMapper.Map<CreateShowtimeCommand>(request)
                );
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "An error occurred at {grpcServiceName}",
                    nameof(ShowtimeService)
                );
                var errorRetModel = new responseModel() { Success = false };
                errorRetModel.Exceptions.Add(
                    new moviesApiException() { Message = ex.Message, StatusCode = 500 }
                );
